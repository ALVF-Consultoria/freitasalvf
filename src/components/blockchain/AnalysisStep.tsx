...
export const AnalysisStep = ({ step }: AnalysisStepProps) => {
   ...
   return (
      <motion.div
         ...
         className="flex flex-col md:flex-row items-center gap-16 w-full text-left font-mono pt-20"
      >
         <div className="md:w-1/2 space-y-12">
            <div className="space-y-2">
               ...
               <h3 className={`text-6xl md:text-[110px] font-black uppercase tracking-tighter leading-none ${platform.color} drop-shadow-[0_0_30px_currentColor]`}>
                  {platform.name}
               </h3>
            </div>

            <div className="grid grid-cols-2 gap-12 border-t border-amber-500/10 pt-8 max-w-lg">
               <div>
                  <span className="block text-[10px] text-white/30 uppercase tracking-widest mb-1">RELATÓRIO_MISSÃO</span>
                  <p className="text-white text-lg font-bold italic leading-tight uppercase tracking-tight">
                     "{platform.mission}"
                  </p>
               </div>
               <div className="space-y-2">
         ...
         {/* Technical Spec Box */}
         <div className={`flex-1 p-12 border ${platform.color} border-current border-opacity-10 bg-white/5 backdrop-blur-xl relative overflow-hidden group`}>
            <div className="absolute top-0 right-0 p-4 opacity-10">
               <Terminal className="w-24 h-24" />
            </div>
            <h4 className="text-xs uppercase tracking-[0.5em] mb-12 opacity-40">DIAGNÓSTICO_SISTEMA</h4>

            <div className="grid grid-cols-1 gap-6 font-mono text-base">
               {platform.specs?.map((spec, i) => (
                  <div key={i} className="flex justify-between items-center border-b border-white/5 pb-2">
                     <span className="text-white/20 text-[10px] tracking-widest uppercase">{spec.l}</span>
