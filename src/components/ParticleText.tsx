...
const waitForFonts = async (font: string): Promise<void> => {
  ...
  try {
    ...
    // A fonte pode nao estar registrada no FontFaceSet; o document.fonts.ready
    // abaixo ainda cobre o caso comum. Amostrar com a fonte errada e melhor do
    // que nao amostrar.
  }
...
export const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async (): Promise<void> => {
      ...
      // clientWidth/clientHeight e nao getBoundingClientRect: o rect inclui os
      // transforms dos ancestrais, e este bloco vive dentro do motion.div da
      // gramatica de viagem, que entra em scale 0.55. Medindo pelo rect, o
      // espaco das particulas nascia 55% menor que o canvas exibido e o ponteiro
      // caia noutra escala.
      ...
      const sampleStep = Math.max(2, Math.floor(density));
    ...
    const handlePointerMove = (event: PointerEvent): void => {
      ...
      // O rect esta em pixels de tela e pode estar escalado por um ancestral; as
      // particulas vivem no espaco nao escalado de width/height. A razao entre os
      // dois converte um no outro, seja qual for o scale no momento.
