...
export interface DepthTextSegment {
  text: string;
  /** Cor so da face. A extrusao continua usando o depthColor unico. */
  color?: string;
}

interface DepthTextProps {
  text?: string;
  /** Texto em pedacos de cores diferentes. Quando presente, ignora `text`. */
  segments?: DepthTextSegment[];
  layers?: number;
  depth?: number;
  faceColor?: string;
  depthColor?: string;
...
export const DepthText = ({
  ...
  const parts: DepthTextSegment[] = segments?.length ? segments : [{ text }];
